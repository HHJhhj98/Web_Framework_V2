...
class TestInvest:
    ...
    # @data(*BD.no100_data)
    # def test_invest_0_failed_no100(self, no100_data):
    #     my_log.info("****异常用例——{}****".format(no100_data['title']))
    #     self.bid.invest(no100_data['amount'])
    #     error_msg = self.bid.get_errorMsg_from_pageCenter()
    #     try:
    #         self.assertEqual(error_msg, no100_data['check'])
    #     except:
    #         my_log.exception("****异常用例——{}，断言失败！！！****".format(no100_data['title']))
    #         raise
    #
    # @data(*BD.no10_data)
    # def test_invest_0_failed_no10(self, no10_data):
    #     my_log.info("****异常用例——{}****".format(no10_data['title']))
    #     self.bid.invest(no10_data['amount'])
    #     error_msg = self.bid.get_errorMsg_from_investButton()
    #     try:
    #         self.assertEqual(error_msg, no10_data['check'])
    #     except:
    ...
    def test_invest_1_success(self, access_web):
        my_log.info("****正常用例——{}****".format(BD.success_data['title']))
        ...
        try:
            # 投资前的金额 -投资后的金额 = 投资金额
            assert float(BD.success_data['amount']) == float(before_amount) - float(after_amount)
            # self.assertEqual(float(BD.success_data['amount']), float(before_amount) - float(after_amount))
        except:
            my_log.exception("****正常用例——{}，断言失败！！！****".format(BD.success_data['title']))
            raise
        # 投资记录对不对？
        # 利息对不对？#单独设计用例
